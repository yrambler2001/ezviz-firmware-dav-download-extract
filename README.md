...
The binary is vendored here as `firmware/digicap_V5.3.8_build_251219.dav`, so
a clone has it already — see [Legal](#legal).
...
└── firmware/
    ...
    ├── unpacked/                        gitignored — regenerated by unpack-dav.sh
    └── rootfs/                          gitignored — regenerated by unpack-dav.sh
```
...
The **extracted** trees (`unpacked/`, `rootfs/`) stay out of version control.
They are derived from the same image and `unpack-dav.sh` regenerates them, and
`rootfs/` carries vendor keys and certificates that have no business in a
public repo.
